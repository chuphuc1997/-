...
import { PackageIcon, CubeIcon, ExclamationTriangleIcon } from './Icons';
...
const DashboardStats: React.FC<DashboardStatsProps> = ({ stats }) => {
  return (
    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
      <StatCard
        icon={<PackageIcon className="h-6 w-6 text-blue-800 dark:text-blue-200" />}
        title="총 상품 종류 (고유 SKU)"
      ...
      />
      <StatCard
        icon={<ExclamationTriangleIcon className="h-6 w-6 text-yellow-800 dark:text-yellow-200" />}
        title="재고 부족 상품"
